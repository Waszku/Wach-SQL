...
2. Jakie jest średnie wynagrodzenie pracowników dla kazdej roli w firmie?

SELECT Rola, AVG(Pensja) as SredniaPensja FROM Pracownicy GROUP BY Rola

3. Ilu pracowników zostało zatrudnionych między 2012 a 2018 rokiem i na jaką pozycję?

SELECT COUNT(rola) as liczba_zatrudnionych, Rola FROM Pracownicy WHERE Data_zatrudnienia BETWEEN '2012-01-01' AND '2017-12-31' 
GROUP BY rola

4. Jakie jest najwyższe a jakie najniższe wynagrodzenie w firmie?

SELECT MAX(Pensja) najwyższa_pensja, MIN(Pensja) as najniższa_pensja FROM Pracownicy 


5. Który pracownik ma najwyższe wynagrodzenie w firmie, a który najniższe?
...
6. Ilu pracowników jest zatrudnionych dłużej niż 5 lat?

SELECT COUNT(*) as pracownik_dluzej_niz_5_lat FROM Pracownicy WHERE data_zatrudnienia <  DATEADD(Year, -5, GETDATE()) 

7. Jakie jest średnie wynagrodzenie dla pracowników zatrudnionych od 2017 roku?

SELECT AVG(Pensja) as srednia_pensja_od2017 FROM Pracownicy WHERE data_zatrudnienia >='2017-01-01'

--- Dodawanie kolejnej kolumny "Plec" do tabeli 

ALTER TABLE Pracownicy 
ADD "Plec" VARCHAR (10)

--- Dodawanie plci dla danego pracownika
...
--- Dodanie nowej kolumny "podwyzki" 

ALTER TABLE Pracownicy
ADD "Podwyzki" INT 

--- Każdy z pracowników zatrudnionych minimum 7 lat dostanie 1000 zł podwyżki, reszta 420 zł
...
--- Ile wynosi pensja pracowników po podwyżkach
...
--- Jakie jest nowe wynagrodzenie po podwyżkach pracowników i o ile procent wzrosło względem starego?
...
--- Którzy pracownicy odnotowali wzrost podwyżki powyżej 20%?
...
--- Wzrost Procentowy po podwyżkach od największego do najmniejszego
